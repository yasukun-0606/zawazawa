...
*/
            //ユーザーID＆日付で検索する
            $sql = "select * from nutritionreg_table where UserID = ? ";                             //SQL文
            $stmt = $pdo->prepare($sql);                                //SQL文のセットとデータベースへ接続
...
*/
            $sql = "select * from momentreg_table where UserID = $id and Date = $date";    //SQL文
            $stmt = $pdo->prepare($sql);     //SQL文のセットとデータベースへ接続
            $stmt->execute([$sql]);             //フォーム情報をSQL文にセットし実行
            ...
            echo "kcal　＜<br/>";
            echo "￣^Y^Y^Y^Y^￣<br/></h3>";
            if($sa<-1000){          //差が-1000未満なら
                echo "<div class='flame01'><p align='center'>ペヤングGIGAMAXぐらいで調子乗るなよwww</p></div>";            
            }elseif($sa<-500){      //差が-500未満なら
                echo "<div class='flame01'><p align='center'>1食抜いたぐらいで調子乗るなよwww</p></div>";
            }elseif($sa>500){       //差が500超なら
                echo "<div class='flame01'><p align='center'>いいんちゃうwwしらんけどww</p></div>";
            }else{                  //差が0なら
